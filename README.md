...
Download the latest release tarball available under
...
```
The git `master` branch always points to the latest stable release while `next`
is an early preview of the next upcoming release (do not use in a production environment).

#
If you update the code, please delete your old build and installation directory. 
...
We try hard not to break userland but the code is evolving quickly so things might change from one version to another without being backward compatible (see RELEASE.md). 

```sh
cd $NEKRS_HOME/examples/turbPipePeriodic
mpirun -np 2 nekrs --setup turbPipe.par
```
For convenience we provide various launch scripts (see bin directory).

## Documentation 
For documentation, see our [readthedocs page](https://nekrs.readthedocs.io/en/latest/). Unfortunately it's just a dummy. We hope to improve documentation to make it more useable for new users. 
